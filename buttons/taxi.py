...
class AddTaxiDialog(QDialog):
    taxi_added = Signal(float)
    ...
    def __init__(self, parent=None):
        ...
        self.a_button.clicked.connect(self.add_taxi_to_db)

    def add_taxi_to_db(self):
        value_text = self.vue_input.text()
        if value_text:
            try:
                value = float(value_text.replace(',', '.'))
                self.taxi_added.emit(value)
                self.accept()

                conn = sqlite3.connect('cash.db')
                cursor = conn.cursor()
                cursor.execute('INSERT INTO expenses (Type, Quantity, Time) VALUES (?, ?, datetime("now", "localtime"))', ('taxi', value))
                conn.commit()
                conn.close()
