...
class AddConnectionDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.a_button.clicked.connect(self.add_connection_to_db)
    ...
    def add_connection_to_db(self):
        ...
        if value_text:
            try:
                ...
                self.connection_added.emit(value)
                self.accept()

                conn = sqlite3.connect('cash.db')
                cursor = conn.cursor()
                cursor.execute('INSERT INTO expenses (Type, Quantity, Time) VALUES (?, ?, datetime("now", "localtime"))', ('connection', value))
                conn.commit()
                conn.close()
